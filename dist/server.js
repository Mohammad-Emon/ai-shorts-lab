...
function mockStream(prompt, write, end) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && parsed.pathname === "/api/stream") {
        ...
        const prompt = String(parsed.query?.q || "Hello");
        const messages = [{ role: "user", content: prompt }];
        const write = (d) => res.write(d);
        const end = () => res.end();
            ...
            try {
                await client.streamChat(messages, (chunk) => {
                    if (chunk.isFinal) {
                        ...
                        write(`data: ${JSON.stringify({ choices: [{ delta: { content: chunk.content } }] })}\n\n`);
                });
            }
            catch (e) {
